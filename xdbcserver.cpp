...
int XDBCServer::serve()
{
    ...
    std::string fileName = "/dev/shm/" + tableName + ".csv";
    auto fileSizeBytes = std::filesystem::file_size(fileName);
    xdbcEnv->tf_paras.bufProcessed = fileSizeBytes / (1024 * xdbcEnv->buffer_size);
    dataSize = 0;
